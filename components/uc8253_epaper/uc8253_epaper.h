...
//
// Supports both refresh modes from the reference driver:
//  - full refresh (update_full_): the slow, flashy full-panel waveform.
//  - partial refresh (update_part_): a gentler/faster waveform that reads
//    the panel's internal previous/current RAM diff, used for most updates.
// No fast-refresh temperature-forcing trick (CCSET/TSSET) is ported, since
// it's a speed optimization only, not needed for correctness.
...
class UC8253EPaper : public display::DisplayBuffer,
 ...
 protected:
  ...
  void update_full_();
  void update_part_();
  void send_partial_window_();
  void send_buffer_to_(uint8_t target_command);
  ...
  // Partial refreshes are gentle/fast but let faint ghosting accumulate over
  // many uses, since they only apply the waveform to pixels the panel's own
  // diff logic thinks changed. Forcing a full refresh every so often clears
  // that - see FULL_REFRESH_EVERY_N_UPDATES in the .cpp for the interval.
  uint32_t updates_since_full_refresh_{0};
};
